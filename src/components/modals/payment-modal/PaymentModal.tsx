...
export function PaymentModal({ closeModal }: { closeModal: () => void }) {
  return (
    ...
    >
      ...
      {/* modal wrapper (ТВОЙ СТИЛЬ НЕ ТРОГАЮ) */}
      <div className="relative z-10 max-w-400 w-full m-0 md:m-5 max-h-[96vh] h-full rounded-[10px] overflow-hidden flex flex-col">

        {/* background */}
        <div className="absolute inset-0 bg-[url('/background/img.jpg')] bg-cover bg-center blur-xl scale-130 pointer-events-none" />
        ...
        {/* SCROLL AREA (ТОЛЬКО ФИКС IOS) */}
        <div
          className="relative z-10 flex-1 overflow-y-auto px-5 xs:px-8 pb-10"
          style={{
            WebkitOverflowScrolling: 'touch', // 🔥 FIX
